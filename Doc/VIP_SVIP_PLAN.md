...
**修订时间**：18:09  
**修订记录**：
- v1.1（14:29）：增加立即水晶奖励、时光沙漏奖励、磁铁与掠夺战特权
...
7. **通用物品口径**：水晶、时光沙漏与VIP/SVIP权益道具统一纳入物品体系管理；其中通用道具ID按 `2=水晶`、`010300001=时光沙漏` 处理
...
|----------|----------|---------|----------|
| **立即获得水晶奖励** | 无 | 180水晶 | 300水晶 |
| **每日水晶奖励** | 无 | 30水晶/日 | 60水晶/日 |
| **每日时光沙漏奖励** | 无 | 无 | 3张/日 |
...
- **奖励叠加**：每日可领取VIP的30水晶 + SVIP的60水晶+3时光沙漏
...
#### VIP月卡
- **立即奖励**：购买VIP立即获得180水晶
- **每日奖励**：每日登录可领取30水晶（价值约3元）
- **领取规则**：激活当日立即获得1次可领取次数，需玩家手动领取；此后每日00:00增加1次可领取次数，可累积3日（防流失）
- **累计价值**：立即180水晶 + 30天共900水晶 = 1080水晶，价值108元，性价比6倍

#### SVIP月卡
- **立即奖励**：购买SVIP立即获得300水晶
- **每日奖励**：每日登录可领取60水晶（价值约6元）+ 时光沙漏×3
- **领取规则**：激活当日立即获得1次可领取次数，需玩家手动领取；此后每日00:00增加1次可领取次数，可累积3日（防流失）
- **时光沙漏价值**：每个时光沙漏价值5水晶
- **累计价值**：立即300水晶 + 30天共1800水晶 + 时光沙漏×90（价值450水晶）= 2550水晶总价值，价值255元，性价比8.5倍
...
|-------------|------|-----------|------|
| 未激活 | 购买VIP月卡 | VIP激活 | 立即创建 `item_vip_active` 权益实例，开始30天倒计时，发放180水晶（`2 x180`），并创建对应权益状态对象且增加当天1次可领取日奖励次数 |
...
|--------------|------|------------|------|
| 未激活 | 购买SVIP月卡 | SVIP激活 | 立即创建 `item_svip_active` 权益实例，开始30天倒计时，发放300水晶（`2 x300`），并创建对应权益状态对象且增加当天1次可领取日奖励次数 |
...
- 每日奖励独立累计并手动领取：VIP和SVIP分别维护各自的权益状态对象，VIP每日30水晶（`2 x30`），SVIP每日60水晶（`2 x60`）+3时光沙漏（`010300001 x3`）
...
为满足“任何东西都当成物品”的实现目标，测试用例统一采用“购买即生成时效权益物品 + 通用物品发奖 + 分离权益状态与玩法日切状态”的口径。VIP、SVIP购买后立即生效，并在背包中存在一个时效权益道具作为权益证明；水晶、时光沙漏也统一视为普通物品；每日奖励累计状态与第二队列开关不再揉进权益实例对象，而是拆分到独立状态对象。
...
| `2` | 水晶 | 普通物品 | 是 | 否 | 否 | 月卡立即奖励与每日奖励都以该物品形式发放 |
| `010300001` | 时光沙漏 | 普通物品 | 是 | 否 | 否 | SVIP每日奖励发放该物品；有广告需求时自动消耗1个跳过广告 |
...
| `itemId` | 物品类型ID，如 `2` / `010300001` |
| `count` | 当前持有数量 |
| `hasExpireAt` | 是否有时效；水晶、时光沙漏为 `false` |
| `expireAt` | 过期时间；无时效时为空 |

说明：
- 可堆叠物品按“同一背包下同一 `itemId`、同一过期批次对应一条记录”处理
- 水晶、时光沙漏都属于普通堆叠物品，不为每一份奖励单独生成实例ID
...
| 自动跳广告 | 当玩法需要广告时，若背包存在 `010300001`，优先自动消耗1个；否则走广告流程 |
...
| 物品上限 | 物品上限校验不在月卡模块处理；月卡模块只负责按配置发放 `2`、`010300001` 等物品 |
...
| C08A | 激活当日手动领取VIP日奖励 | 存在刚激活的 `item_vip_active`；对应权益状态对象 `pendingClaimDays=1`；`2=D0` | 手动领取当天VIP日奖励 | `pendingClaimDays=0`；`2=D0+30` |
| C08B | 激活当日手动领取SVIP日奖励 | 存在刚激活的 `item_svip_active`；对应权益状态对象 `pendingClaimDays=1`；`2=D0`；`010300001=H0` | 手动领取当天SVIP日奖励 | `pendingClaimDays=0`；`2=D0+60`；`010300001=H0+3` |
| C09 | 领取VIP每日奖励 | 存在未过期 `item_vip_active`；对应权益状态对象 `pendingClaimDays=1`；`2=D0` | 领取VIP每日奖励 | `pendingClaimDays=0`；`2=D0+30` |
| C10 | 领取SVIP每日奖励 | 存在未过期 `item_svip_active`；对应权益状态对象 `pendingClaimDays=1`；`2=D0`；`010300001=H0` | 领取SVIP每日奖励 | `pendingClaimDays=0`；`2=D0+60`；`010300001=H0+3` |
| C11 | 双月卡一键领取 | 同时存在未过期 `item_vip_active` 和 `item_svip_active`；两个权益状态对象 `pendingClaimDays=1`；`2=D0`；`010300001=H0` | 一键领取月卡奖励 | 两个权益状态对象的 `pendingClaimDays` 各减1；`2=D0+90`；`010300001=H0+3` |
...
| C15 | 时光沙漏自动跳过广告 | 玩家需要触发广告玩法；背包有 `010300001` x2 | 触发一次需要广告的复活/磁铁/掠夺战补次 | 自动消耗 `010300001` x1；本次不进入广告流程；对应玩法成功结算 |
| C16 | 免费玩家磁铁使用 | 无VIP；背包无时光沙漏 | 战斗中触发磁铁 | 必须走广告校验；广告完成后磁铁生效 |
| C17 | VIP玩家磁铁使用 | 存在未过期 `item_vip_active` | 战斗中触发磁铁 | 不需要广告；不消耗时光沙漏；磁铁直接生效 |
| C18 | SVIP玩家磁铁使用 | 存在未过期 `item_svip_active`；背包有 `010300001` x1 | 战斗中触发磁铁 | 按“需广告”口径判定；自动消耗 `010300001` x1；磁铁生效 |
...
| B08 | 同一购买请求重复提交 | 不存在对应权益道具；客户端因超时重发同一购买请求 | 服务端收到同一购买请求两次 | 只创建1个权益道具；立即水晶只增加1次；有效期只记录1次 |
| B09 | 同一奖励领取请求重复提交 | 存在未过期权益物品；对应权益状态对象 `pendingClaimDays=1` | 服务端收到两次领取请求 | 仅第一次成功；`pendingClaimDays` 只减1次；`2`、`010300001` 等奖励物品只增加1次 |
...
| B14 | 时光沙漏不足时广告回退 | 玩法需要广告；背包 `010300001` x0 | 触发广告玩法 | 不可跳过广告，必须进入广告校验流程 |
| B15 | 免费玩家持有时光沙漏 | 免费玩家无任何月卡；背包 `010300001` x1 | 触发一次需要广告的磁铁/复活/掠夺战补次 | 自动消耗 `010300001` x1；本次跳过广告；对应玩法成功 |
...
| B21 | 物品接近上限时发立即奖励 | 购买月卡前 `2` 接近上限 | 购买月卡 | 月卡模块仍按配置发放立即奖励物品；`2` 是否溢出、截断或转邮件由物品模块处理 |
| B22 | 物品接近上限时领SVIP每日奖励 | 存在未过期 `item_svip_active`；对应权益状态对象 `pendingClaimDays=1`；`2` 或 `010300001` 接近上限 | 领取SVIP每日奖励 | 月卡模块仍按配置发放 `2 x60` 和 `010300001 x3`；物品上限处理由物品模块负责 |
